...
`proxy-status`、`proxy-overview`、`proxy-readiness`、`proxy-assert-ready`、`proxy-snapshot`、`proxy-diagnostics`、`proxy-attention`、`proxy-incidents`、`proxy-handoff`、`proxy-timeline`、`proxy-triage`、`proxy-casebook`、`proxy-board`、`proxy-focus`、`proxy-lens`、`proxy-brief`、`proxy-dossier`、`proxy-runbook`、`proxy-mission`、`proxy-workspace`、`proxy-cockpit`
...
- dashboard payload に operator cockpit catalog も追加し、`proxy-cockpit` と control-plane の `/cockpit`、`/routes/{id}/cockpit`、`/destinations/{id}/cockpit` から、focus、mission、workspace、runbook、blocker、board title、work-item title、next step action をひとつに束ねた高密度な triage contract を取得できるようにした
